...
GET http://localhost:8084/api/events/{eventId}

# 2. Réduire les places lors d'une réservation
POST http://localhost:8084/api/events/internal/{eventId}/decrease-seats?quantity=5

# 3. Libérer les places lors d'une annulation
POST http://localhost:8084/api/events/internal/{eventId}/increase-seats?quantity=5
```
...
GET http://localhost:8084/api/events/search?organisateurId={userId}
```
...
```bash
curl -X PUT http://localhost:8084/api/events/1 \
...
```bash
curl -X POST http://localhost:8084/api/events/1/cancel \
  -H "Content-Type: application/json" \
